...
class _AdminPriceEditPageState extends State<AdminPriceEditPage>
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (mounted) AppSnackbar.showError(context, 'فشل تحميل البيانات.');
    }
  }
  ...
  Future<void> _editProductPrice(SimpleProductModel product) async {
    ...
    final confirmed = await _confirmChange(product.name, product.salePrice, newPrice);
    ...
    try {
      ...
      if (mounted) {
        AppSnackbar.showError(context, e.response?.data?['message'] as String? ?? 'فشل تحديث السعر.');
      }
    } catch (_) {
  ...
  Future<void> _editRawMaterialPrice(RawMaterialModel material) async {
    ...
    if (newPrice == null) return;
    final confirmed = await _confirmChange(material.name, material.costPrice, newPrice);
    if (confirmed != true) return;
    try {
      final result = await _remote.updateRawMaterialPrice(material.id, newPrice);
      if (!mounted) return;
      AppSnackbar.showSuccess(context,
          'تم تغيير سعر ${result.name} من ${result.oldPrice.toStringAsFixed(2)} إلى ${result.newPrice.toStringAsFixed(2)}');
      _load();
    ...
    return showDialog<bool>(
      ...
      builder: (dialogCtx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(onPressed: () => Navigator.pop(dialogCtx, true), child: const Text('تأكيد')),
        ],
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filteredMaterials =
        _rawMaterials.where((m) => m.name.toLowerCase().contains(_search.toLowerCase())).toList();
          ...
          if (_loading)
            ...
            Expanded(
              child: TabBarView(
                controller: _tabController,
                children: [
                  _ProductList(products: filteredProducts, onTap: _editProductPrice),
                  _RawMaterialList(materials: filteredMaterials, onTap: _editRawMaterialPrice),
                ],
              ),
            ),
        ],
      ...
      itemBuilder: (_, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text('سعر الجملة الحالي: ${p.salePrice.toStringAsFixed(2)}',
                style: const TextStyle(fontSize: 12)),
            trailing: const Icon(Icons.edit_outlined, color: AppTheme.secondary),
            onTap: () => onTap(p),
          ),
        );
      },
      ...
      itemBuilder: (_, i) {
        ...
        return Card(
          ...
          child: ListTile(
            ...
            subtitle: Text('سعر التكلفة الحالي: ${m.costPrice.toStringAsFixed(2)} / ${m.unit}',
                style: const TextStyle(fontSize: 12)),
            trailing: const Icon(Icons.edit_outlined, color: AppTheme.secondary),
            onTap: () => onTap(m),
          ),
