...
class AdminSupplierPaymentPage extends StatefulWidget {
  ...
  @override
  State<AdminSupplierPaymentPage> createState() => _AdminSupplierPaymentPageState();
}
...
class _AdminSupplierPaymentPageState extends State<AdminSupplierPaymentPage> {
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      AppSnackbar.showError(context, e.response?.data?['message'] as String? ?? 'فشل تسجيل السداد.');
    } catch (_) {
      setState(() => _submitting = false);
          ...
          : ListView(
              ...
              children: [
                DropdownButtonFormField<int>(
                  ...
                  items: _suppliers
                      .map((s) => DropdownMenuItem(
                          value: s.id, child: Text('${s.name}${s.balance > 0 ? ' (مديونية: ${s.balance.toStringAsFixed(0)})' : ''}')))
                      .toList(),
                ...
                TextField(
                  ...
                  decoration: const InputDecoration(labelText: 'المبلغ المسدَّد', hintText: '0'),
                ),
                const SizedBox(height: 12),
                DropdownButtonFormField<int>(
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    ...
                    label: Text(_submitting ? 'جارٍ الحفظ...' : 'حفظ السداد'),
                  ),
                ),
              ],
            ),
