...
class _StaffOperationsPageState extends State<StaffOperationsPage> {
  ...
  Future<void> _loadStaff() async {
    ...
    try {
      ...
      AppSnackbar.showError(context, 'فشل تحميل قائمة العمالة.');
    }
  }
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (!mounted) return;
      AppSnackbar.showSuccess(context, 'تم تسجيل العملية بنجاح — ${staff.name}.');
      setState(() {
        _submitting = false;
        _selectedStaff = null;
        _searchCtrl.clear();
      ...
      AppSnackbar.showError(
          context, e.response?.data?['message'] as String? ?? 'فشل تسجيل العملية.');
    } catch (_) {
      if (!mounted) return;
      setState(() => _submitting = false);
                ...
                if (_selectedStaff == null) ...[
                          ...
                          child: ListTile(
                            leading: CircleAvatar(
                              ...
                              child: Icon(
                                ...
                                color: s.workerType == 'worker' ? Colors.amber.shade800 : AppTheme.primary,
                              ),
                            ),
                            title: Text(s.name),
                            subtitle: Text(s.workerType == 'worker' ? 'عامل إنتاج' : 'مندوب مبيعات'),
                  ...
                  Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                          _selectedStaff!.workerType == 'worker' ? 'عامل إنتاج' : 'مندوب مبيعات'),
                      trailing: TextButton(
                  ...
                  Text('نوع العملية', style: Theme.of(context).textTheme.labelLarge),
                    ...
                    children: [
                      Expanded(
                        child: ChoiceChip(
                          ...
                          onSelected: (_) => setState(() => _operationType = 'penalty'),
                        ),
                      ),
                      ...
                      Expanded(
                        child: ChoiceChip(
                          ...
                          onSelected: (_) => setState(() => _operationType = 'advance'),
                        ),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: ChoiceChip(
                          ...
                          onSelected: (_) => setState(() => _operationType = 'bonus'),
                        ),
                      ),
                    ],
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text(_submitting ? 'جارٍ الحفظ...' : 'تسجيل العملية'),
                    ),
                  ),
                ],
              ],
