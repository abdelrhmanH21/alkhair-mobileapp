...
class _StartProductionPageState extends State<StartProductionPage> {
  ...
  Future<void> _onCategoryChanged(int? categoryId) async {
    ...
    try {
      ...
      setState(() {
        _loadingRecipes = false;
        _recipesError = 'فشل تحميل الوصفات.';
      });
    }
  }
  ...
  Future<void> _submit() async {
    ...
    final activeInputs = _inputRows
        .where((r) =>
            r._qtyCtrl.text.isNotEmpty &&
            (double.tryParse(r._qtyCtrl.text) ?? 0) > 0)
        .toList();
    ...
    if (overStock.isNotEmpty) {
      AppSnackbar.showError(context,
          'مخزون غير كافٍ: ${overStock.map((r) => r.recipeInput.name).join('، ')}');
      return;
    }
    ...
    try {
      ...
      AppSnackbar.showError(context,
          e.response?.data?['message'] as String? ?? 'فشل فتح التشغيلة');
    } catch (_) {
      setState(() => _submitting = false);
              ...
              : RefreshIndicator(
                  ...
                  child: ListView(
                    ...
                    children: [
                      ...
                      if (_loadingRecipes)
                        const Center(child: CircularProgressIndicator()),
                      if (!_loadingRecipes && _recipesError != null)
                        AppErrorView(
                          message: _recipesError!,
                          onRetry: () => _onCategoryChanged(_categoryId),
                        ),
                      if (!_loadingRecipes &&
                          _recipesError == null &&
                          _categoryId != null &&
                      ...
                      if (_selectedRecipe != null) ...[
                        ...
                        TextField(
                          ...
                          keyboardType: const TextInputType.numberWithOptions(
                              decimal: true),
                          decoration: const InputDecoration(
                              labelText: 'مصروفات إضافية للتشغيلة (اختياري)',
                              hintText: '0.00'),
                          onChanged: (_) => setState(() {}),
                        ),
                        if ((double.tryParse(_expenseCtrl.text) ?? 0) > 0 &&
                            _treasuries.isNotEmpty) ...[
                          const SizedBox(height: 8),
                          DropdownButtonFormField<int>(
                            initialValue: _expenseTreasuryId,
                            decoration:
                                const InputDecoration(labelText: 'الخزينة'),
                            items: _treasuries
                                .map((t) => DropdownMenuItem(
                                    value: t.id, child: Text(t.name)))
                                .toList(),
                            onChanged: (v) =>
                                setState(() => _expenseTreasuryId = v),
                          ),
                        ],
                        ...
                        const Text(
                            'ستُخصم فوراً من المخزون. اترك فارغاً للمكوّنات غير المستخدمة.',
                            style: TextStyle(
                                      ...
                                      children: [
                                        Text(row.recipeInput.name,
                                            style: const TextStyle(
                                                ...
                                                fontSize: 13)),
                                        if (row.recipeInput.currentStock !=
                                            null)
                                          Text(
                                              'المتاح: ${row.recipeInput.currentStock!.toStringAsFixed(2)} ${row.recipeInput.unit}',
                                              style: TextStyle(
                                                  fontSize: 11,
                                                  color: warn
                                                      ? AppTheme.danger
                                ...
                                child: Column(
                                  children: [
                                    DropdownButtonFormField<int>(
                                      ...
                                      decoration: const InputDecoration(
                                          labelText: 'المنتج', isDense: true),
                                      items: _allProducts
                                          .map((p) => DropdownMenuItem(
                                              value: p.id, child: Text(p.name)))
                                          .toList(),
                                      onChanged: (v) =>
                                          setState(() => row.productId = v),
                                    ),
                                    const SizedBox(height: 8),
                                    Row(
                                      children: [
                        ...
                        const SizedBox(height: 4),
                        const Text(
                            'الكميات الفعلية تُسجَّل لاحقاً في «استلام إنتاج تام»',
                            style: TextStyle(
                                fontSize: 11, color: AppTheme.textMuted)),
                        const SizedBox(height: 8),
                        ...
                        SizedBox(
                          ...
                          child: ElevatedButton.icon(
                            ...
                            label: Text(_submitting
                                ? 'جارٍ فتح التشغيلة...'
                                : 'فتح التشغيلة وصرف الخامات'),
                          ),
                        ),
                      ],
                      const SizedBox(height: 24),
                      Text('التشغيلات الجارية',
                          style: Theme.of(context).textTheme.titleMedium),
                      const SizedBox(height: 8),
                      if (_recentBatches.isEmpty)
                        const Padding(
                          padding: EdgeInsets.symmetric(vertical: 8),
                          child: Text('لا توجد تشغيلات جارية',
